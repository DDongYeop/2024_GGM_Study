...
public class GameManager : ISingleton<GameManager>
{
    ...
    public void BroadcastAttack(Character attacker, Character target, int skillId, float direction,
                            FLocation pos, FLocation dest, FLocation postHitPosition, float postHitDirection, int damage, EAttackResult result, float timeOffset, bool killFlag)
    {
        EKillType isKillType = EKillType.False;
        if (killFlag == true)
        {
            isKillType = EKillType.True;
        }
        var broadcastPacket = new PacketBase();
        ZoneUpdateAck zoneUpdateAck = new ZoneUpdateAck
        {
            ...
            Attacks = new List<AttackBr>
            {
                new AttackBr
                {
                    SrcObjectType = attacker is Pc ? EObjectType.Pc : EObjectType.Npc,
                    SrcObjectIndex = attacker.Index,
                    TarObjectType = target is Pc ? EObjectType.Pc : EObjectType.Npc,
                    TarObjectIndex = target.Index,
                    SkillId = skillId,
                    Direction = direction,
                    Pos = pos,
                    Dest = dest,
