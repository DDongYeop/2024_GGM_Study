...
namespace Process
{
    ...
    public class PacketHandler
    {
        ...
        private void HandleMoveReq(MemoryStream packetStream, Socket handler, int ClientId, long PacketIndex)
        {
            ...
            pc.Pos = new CFLocation { X = moveReq.Dest.X, Y = moveReq.Dest.Y, Z = moveReq.Dest.Z };
        ...
        private void HandleAttackReq(MemoryStream packetStream, Socket handler, int ClientId, long PacketIndex)
        {
            ...
            foreach (var attackInfo in attackReq.AttackInfos)
            {
                Console.WriteLine(
                    ...
                    $"AttackDamage: {attackInfo.AttackDamage}, TimeOffset: {attackInfo.TimeOffset}");

                CObject targetObject = gameManager.GetObject(attackInfo.TargetIndex);
                if (targetObject is Character targetCharacter)
                {
                    // 실제 공격 로직 구현
                    EAttackResult resultType = ProcessAttack(pc, targetCharacter, attackInfo.SkillId);
                    int damageAmount = attackInfo.AttackDamage; // 클라이언트에서 전달된 데미지 사용

                    // 대상의 HP 감소 및 사망 처리
                    EAttackResult damageResult = targetCharacter.TakeDamage(damageAmount, out int out_damage, out bool KillFlag);

                    // 최종 결과 결정 (ProcessAttack의 결과와 실제 데미지 적용 결과 중 더 심각한 것 선택)
                    resultType = (damageResult > resultType) ? damageResult : resultType;

                    attackAck.Results.Add(new AttackResult
                    {
                        Result = resultType,
                        TarObjectType = EObjectType.Npc,
                        //    TarObjectType = targetCharacter is Npc ? EObjectType.Npc : EObjectType.Pc,
                        TarObjectIndex = attackInfo.TargetIndex,
                        TarDamage = out_damage
                    });
                    ...
                    gameManager.BroadcastAttack(pc, targetCharacter, attackInfo.SkillId, attackInfo.Direction,
                                                attackInfo.Pos, attackInfo.Dest, attackInfo.PostHitPosition, attackInfo.PostHitDirection, out_damage, resultType, attackInfo.TimeOffset, KillFlag);
                }
            }
        ...
        private EAttackResult ProcessAttack(Character attacker, Character target, int skillId)
        {
        ...
        private int CalculateDamage(Character attacker, Character target, int skillId)
        {
            // 여기에 실제 데미지 계산 로직을 구현합니다.
            // 예를 들어, 공격력, 방어력, 스킬 효과 등을 고려할 수 있습니다.
            // 간단한 예시로 고정 데미지를 반환합니다.
            return 50;
        }
    }
}
